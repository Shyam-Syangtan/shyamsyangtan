-- ADD SAMPLE DATA V2 - Fixed foreign key constraint issues
-- This creates realistic tutor profiles without foreign key violations

-- Clear existing sample data first
DELETE FROM public.tutors WHERE email LIKE '%@example.com' OR email LIKE '%@tutorapp.com';

-- Create realistic tutor profiles without user_id foreign key constraints
...
-- Add some sample reviews for the tutors (using tutor IDs instead of user_id foreign keys)
...
-- Add some sample availability for tutors (using tutor IDs)
...
-- Create some basic user profiles for testing (without foreign key constraints)
INSERT INTO public.users (email, full_name, role, raw_user_meta_data)
SELECT 
    t.email,
    t.name,
    'tutor',
...
-- Success message
SELECT 'SAMPLE DATA ADDED SUCCESSFULLY V2!' as status,
